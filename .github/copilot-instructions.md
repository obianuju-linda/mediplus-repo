...
- Big picture
  ...
  - CI: `.github/workflows/cicd.yml` contains a commented-out `build-push` job (ECR) and an active `deploy` job that SSHes to a remote host using the `SSH_KEY` secret.
...
- CI/CD & deployment notes (important)
  - File: `.github/workflows/cicd.yml`
    ...
    - Security note: do NOT print private keys in logs (the workflow previously did `cat Dockerhost.pem`). Instead add the host to `~/.ssh/known_hosts` (via `ssh-keyscan`) and avoid exposing secret contents.
...
- Integration points to be aware of
  - Google Maps: see `js/map-active.js` — the script uses GMaps and targets the DOM element with id #myMap. Ensure any Google API keys are injected in templates or replaced where they exist (no key present in repo). Missing API key will break maps on the site.
...
- CI/CD & deployment notes (important)
  - File: `.github/workflows/cicd.yml`
    - The `build-push` job that logs into AWS ECR is commented out. To enable container publish, uncomment and ensure `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` exist in repository secrets.
    - The `deploy` job expects `secrets.SSH_KEY`. It writes this secret to `Dockerhost.pem` and SSHes to `ubuntu@3.94.110.160` to run `docker ps` and `hostnamectl`.
    - Security note: do NOT `cat` private keys in logs (the workflow currently does `cat Dockerhost.pem`). Replace with safer host verification (add host to `known_hosts`) and use `ssh -o StrictHostKeyChecking=no -i Dockerhost.pem` only when necessary.
...
- Integration points to be aware of
  - Google Maps: `js/map-active.js` uses GMaps and references `#myMap`. Ensure any Google API keys are injected in templates or replaced where they exist (no key present in repo). Missing API key will break maps on the site.
  - Deployment target: the workflow SSHs to a specific IP (3.94.110.160). Treat this host as production and coordinate with repository owners before making changes to the deploy steps.
